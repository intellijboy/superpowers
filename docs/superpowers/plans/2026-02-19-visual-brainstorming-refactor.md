# 可视化 Brainstorm 重构实现计划

> **对于 agentic workers：** 必需：使用 superpowers:subagent-driven-development（如果 subagent 可用）或 superpowers:executing-plans 来实现此计划。步骤使用复选框（`- [ ]`）语法进行跟踪。

**目标：** 将可视化 brainstorming 从阻塞式 TUI 反馈模型重构为非阻塞式"浏览器显示，终端命令"架构。

**架构：** 浏览器成为交互式显示；终端保持对话通道。服务器将用户事件写入每个屏幕的 `.events` 文件，Claude 在下一轮读取。消除 `wait-for-feedback.sh` 和所有 `TaskOutput` 阻塞。
...
| `lib/brainstorm-server/helper.js` | 修改 | 客户端 JS：移除 send/feedback 函数，简化为点击捕获 + 指示器更新 |
| `lib/brainstorm-server/wait-for-feedback.sh` | 删除 | 不再需要 |
| `skills/brainstorming/visual-companion.md` | 修改 | 技能说明：将循环重写为非阻塞流程 |
...
用选择指示器栏替换 feedback-footer div（第 227-233 行）：
...
在 `tests/brainstorm-server/server.test.js` 的测试 4 区域后添加 — 一个新测试，发送带有 `choice` 字段的 WebSocket 事件并验证 `.events` 文件被写入：

```javascript
    // Test: 选择事件写入 .events 文件
    ...
    assert(fs.existsSync(eventsFile), '.events 文件应在选择点击后存在');
...
- [ ] **Step 3:** 为 `.events` 文件在新屏幕时清除编写失败测试
...
```javascript
    ...
    // .events 文件应该仍存在于上一个测试中
    assert(fs.existsSync(path.join(TEST_DIR, '.events')), '.events 在新屏幕前应存在');
    fs.writeFileSync(path.join(TEST_DIR, 'new-screen.html'), '<h2>New screen</h2>');
    await sleep(500);
    assert(!fs.existsSync(path.join(TEST_DIR, '.events')), '.events 在新屏幕后应被清除');
    console.log('  PASS');
```
...
删除 `window.send` 函数（第 120-129 行）— 与已移除的发送按钮相关联。

- [ ] **Step 3:** 移除表单提交和输入更改处理器

删除表单提交处理器（第 57-71 行）和输入更改处理器（第 73-89 行），包括 `inputTimeout` 变量。

- [ ] **Step 4:** 移除 `pageshow` 事件监听器

删除我们之前添加的 `pageshow` 监听器（不再有需要清除的 textarea）。

- [ ] **Step 5:** 将点击处理器简化为仅 `[data-choice]`

用更简单的版本替换点击处理器（第 36-55 行）：
...
**注意：** 下面的行引用来自_原始_文件。Task 2 在文件较早位置插入了新测试，所以实际行号会有偏移。通过 `console.log` 标签查找测试（如"Test 5:"、"Test 6:"）。

- [ ] **Step 1:** 更新测试 5（完整文档断言）

找到测试 5 断言 `!fullRes.body.includes('feedback-footer')`。将其更改为：完整文档也不应该有指示器栏（它们按原样提供服务）：
...
第 125 行：用指示器栏断言替换 `feedback-footer` 断言：

```javascript
    assert(fragRes.body.includes('indicator-bar'), '片段应该从 frame 获得指示器栏');
```
...
- [ ] **Step 4:** 用指示器栏测试替换测试 8（sendToClaude 主题）

替换测试 8（第 145-149 行）— `sendToClaude` 不再存在。改为测试指示器栏：

```javascript
    // Test 8: 指示器栏使用 CSS 变量（主题支持）
    ...
    assert(templateContent.includes('indicator-bar'), '模板应有指示器栏');
    assert(templateContent.includes('indicator-text'), '模板应有指示器文本元素');
    console.log('  PASS');
```
...
预期引用：只有 `visual-companion.md`（在 Task 6 中重写）和可能的发布说明（历史性的，保持原样）。
...
用以下内容替换关于"以 JSON 接收反馈"的句子：

```markdown
服务器监视 HTML 文件的目录并将最新的文件提供给浏览器。你编写 HTML 内容，用户在浏览器中查看并可以点击选择选项。选择记录到 `.events` 文件中，你在下一轮读取。
```

- [ ] **Step 2:** 更新片段描述（第 20 行）

从 frame 模板提供的功能描述中移除"feedback footer"：

```markdown
**内容片段 vs 完整文档：** 如果你的 HTML 文件以 `<!DOCTYPE` 或 `<html` 开头，服务器按原样提供（只注入 helper 脚本）。否则，服务器自动将你的内容包装在 frame 模板中 — 添加头部、CSS 主题、选择指示器和所有交互基础设施。**默认编写内容片段。** 只有在需要完全控制页面时才编写完整文档。
```
...
1. **编写 HTML** 到 `screen_dir` 中的新文件：
   ...
   - 使用 Write 工具 — **永远不要使用 cat/heredoc**（会将噪音输出到终端）
   - 服务器自动提供最新的文件

2. **告诉用户期待什么并结束你的回合：**
   - 提醒他们 URL（每一步都要，不只是第一次）
   - 给出屏幕上内容的简短文本摘要（如"显示主页的 3 种布局选项"）
   - 请他们在终端中回复："看看并告诉我你的想法。如果你想的话可以点击选择一个选项。"

3. **在你的下一回合** — 用户在终端回复后：
   - 如果 `$SCREEN_DIR/.events` 存在则读取它 — 这包含用户的浏览器交互（点击、选择）作为 JSON 行
   - 与用户的终端文本合并以获得完整画面
   - 终端消息是主要反馈；`.events` 提供结构化的交互数据

4. **迭代或推进** — 如果反馈改变当前屏幕，编写新文件（如 `layout-v2.html`）。只有当前步骤验证后才移动到下一个问题。
...
```markdown
只编写放入页面内的内容。服务器自动将其包装在 frame 模板中（头部、主题 CSS、选择指示器和所有交互基础设施）。
```

- [ ] **Step 6:** 更新参考部分（第 200-203 行）

移除 helper.js 参考描述中的"JS API" — API 现在很精简。保留路径引用：

```markdown
## 参考

- Frame 模板（CSS 参考）：`${CLAUDE_PLUGIN_ROOT}/lib/brainstorm-server/frame-template.html`
- Helper 脚本（客户端）：`${CLAUDE_PLUGIN_ROOT}/lib/brainstorm-server/helper.js`
```
...
预期：除了发布说明和 spec/plan 文档（它们是历史性的）之外没有命中。

- [ ] **Step 4:** 如需要清理则最终提交

```bash
git status
# 检查未跟踪/修改的文件，按需暂存特定文件，如果干净则提交
```
