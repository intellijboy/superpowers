...
**目标：** 用单个零依赖的 `server.js` 替换 brainstorm 服务器的 vendored node_modules，使用 Node 内置模块。

**架构：** 单文件，包含 WebSocket 协议（RFC 6455 文本帧）、HTTP 服务器（`http` 模块）和文件监视（`fs.watch`）。导出协议函数供作为模块引用时进行单元测试。
...
- **创建：** `skills/brainstorming/scripts/server.js` — 零依赖替换
...
客户端帧总是掩码的。返回 `{ opcode, payload, bytesConsumed }` 或 `null` 表示不完整。在未掩码帧上抛出异常。
...
- [ ] **Step 1: 在 server.js 顶部（requires 之后）添加配置和常量**
...
在模块作用域加载 `frameTemplate` 和 `helperInjection`，以便 `wrapInFrame` 和 `handleRequest` 可以访问它们。它们只从 `__dirname`（scripts 目录）读取文件，无论模块是被引用还是直接运行都有效。
...
文件监视逻辑内联在 `startServer`（Step 6）中，以保持 watcher 生命周期与服务器生命周期一致，并按 spec 包含 `error` 处理器。

- [ ] **Step 6: 添加 startServer 函数和条件主入口**

`frameTemplate` 和 `helperInjection` 已经在模块作用域（Step 2）。`startServer` 只是创建 screen 目录、启动 HTTP 服务器、watcher 并记录启动信息。
...
测试目录已经有 `package.json`，其中 `ws` 作为依赖。如需要则安装，然后运行测试。
...
- [ ] **Step 2: 移除 gitignore 中 node_modules 的例外**
...
预期：打印带端口 9876 的 `server-started` JSON

- [ ] **Step 2: 在浏览器打开 http://localhost:9876**

预期：显示带"Waiting for Claude to push a screen..."的等待页面
...
预期：浏览器刷新并显示包装在 frame 模板中的"Hello from smoke test"
