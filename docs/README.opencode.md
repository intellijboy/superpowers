...
注意：Git Bash 的原生 `ln` 命令会复制文件而不是创建符号链接。请使用 `cmd //c mklink`（`//c` 是 Git Bash 中 `/c` 的语法）。
...
如果在 WSL 中运行 OpenCode，请使用 [macOS / Linux](#macos--linux) 的说明。
...
Superpowers 使用 OpenCode 的原生 `skill` 工具进行技能发现和加载。技能通过符号链接到 `~/.config/opencode/skills/superpowers/`，因此它们会与你的个人和项目技能一起显示。
...
- `TodoWrite` → `todowrite`
- 带有 subagent 的 `Task` → OpenCode 的 `@mention` 系统
...
**组件：**
- `experimental.chat.system.transform` hook 用于引导程序注入
...
3. 检查技能结构：每个技能需要一个带有有效 frontmatter 的 `SKILL.md` 文件

### Windows：模块未找到错误

如果在 Windows 上看到 `Cannot find module` 错误：
- **原因：** Git Bash 的 `ln -sf` 会复制文件而不是创建符号链接
- **修复：** 使用 `mklink /J` 目录连接点（参见 Windows 安装步骤）
...
# 检查技能可发现
opencode run "使用 skill 工具列出所有技能" 2>&1 | grep -i superpowers

# 检查引导程序注入
opencode run "你有什么 superpowers？"
```
