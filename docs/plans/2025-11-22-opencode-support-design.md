...
OpenCode.ai 是一个类似于 Claude Code 和 Codex 的编码代理。之前尝试将 superpowers 移植到 OpenCode（PR #93、PR #116）使用了文件复制方法。本设计采用不同的方法：使用其 JavaScript/TypeScript 插件系统构建原生 OpenCode 插件，同时与 Codex 实现共享代码。

### 平台之间的关键差异

- **Claude Code**：原生 Anthropic 插件系统 + 基于文件的技能
- **Codex**：无插件系统 → 引导 markdown + CLI 脚本
- **OpenCode**：带有事件 hook 和自定义工具 API 的 JavaScript/TypeScript 插件

### OpenCode 的代理系统

- **主代理**：Build（默认，完全访问）和 Plan（受限，只读）
- **Subagent**：General（研究、搜索、多步骤任务）
- **调用**：由主代理自动派遣 或 手动 `@mention` 语法
...
---
name: skill-name
description: 当 [条件] 时使用 - [功能描述]；[附加上下文]
---
```
...
3. **注入工具映射说明**
   ...
   **OpenCode 工具映射：**
   ...
   - 带有 subagent 的 `Task` → 使用 OpenCode subagent 系统（@mention）
