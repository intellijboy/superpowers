...
通过为每个任务派遣新子代理来执行计划，每个任务后进行两阶段审查：先规格合规性审查，然后代码质量审查。

**为什么使用子代理：** 你将任务委托给具有隔离上下文的专业代理。通过精确构建他们的指令和上下文，你确保他们保持专注并成功完成任务。他们永远不应继承你会话的上下文或历史 — 你要准确构建他们需要的内容。这也为你自己的协调工作保留了上下文。
